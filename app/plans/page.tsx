...
function PlanCard({ plan, isLoading, onSelect }: PlanCardProps) {
  return (
    <Card
      className={`relative p-8 hover:scale-105 transition-all duration-300 hover:shadow-2xl ${
    ...
    >
      ...
      <div className="text-center mb-6">
        <div className={`inline-flex w-16 h-16 items-center justify-center rounded-full bg-gradient-to-br ${plan.gradient} mb-4`}>
...
export default function Plans() {
  ...
  const handleSubscribe = useCallback(async (planType: PlanType) => {
    try {
    ...
    <div className="min-h-screen pt-16 bg-gradient-to-br from-background via-background to-primary/5">
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid md:grid-cols-3 gap-8">
          {PLANS.map((plan, i) => (
            <div key={plan.type} className="animate-scale-in" style={{ animationDelay: `${i * 0.1}s` }}>
              <PlanCard plan={plan} isLoading={isLoading} onSelect={handleSubscribe} />
            </div>
          ))}
        </div>
