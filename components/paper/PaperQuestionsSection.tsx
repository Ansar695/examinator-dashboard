...
interface PaperQuestionsSectionProps {
  ...
  mcqMarks?: number;
  startIndex: number;
  onQuestionEdit: (type: 'mcq' | 'short' | 'long', id: string, newText: string) => void;
  onMCQOptionEdit?: (questionId: string, optionIndex: number, newText: string) => void;
        ...
        {sectionType === 'mcq' && onMcqMarksChange && (
          <div className="flex items-center space-x-2">
            ...
            <Input
              ...
              className="w-20"
              value={mcqMarks || ''}
              onChange={(e) => onMcqMarksChange(e.target.value === '' ? undefined : Number(e.target.value))}
            />
          </div>
        )}
        {sectionType !== 'mcq' && (
          <span className="text-sm font-medium">Total: {calculateSectionMarks()} Marks</span>
        )}
      </div>

      <div className="space-y-4">
        {questions.map((question, index) => (
          <div key={question.id} className={sectionType === 'mcq' ? "pl-4" : "flex justify-between items-start pl-4"}>
            <div className={sectionType !== 'mcq' ? "flex-grow" : ""}>
              <EditableQuestion
                initialText={`${startIndex + index}. ${question.text}`}
                onSave={(newText) => onQuestionEdit(sectionType, question.id, newText.replace(/^\d+\.\s*/, ''))}
              />
              
              {/* MCQ Options */}
              {sectionType === 'mcq' && question.options && onMCQOptionEdit && (
                <div className="pl-8 mt-2">
                  {question.options.map((option, optionIndex) => (
                    <div key={optionIndex} className="flex items-center space-x-2">
                      ...
                      <EditableMcq
                        initialText={option}
                        onSave={(newText) => onMCQOptionEdit(question.id, optionIndex, newText)}
                      />
                    </div>
                  ))}
                </div>
              ...
              {/* Long Question Parts */}
              {sectionType === 'long' && question.parts && question.parts.length > 0 && (
                  ...
                  {question.parts.map((part, partIndex) => (
                    <div key={partIndex} className="flex justify-between items-start">
                      ...
                      <Input
                        ...
                        onChange={(e) => onMarksChange(`${question.id}-${partIndex}`, e.target.value)}
                      />
                    </div>
                  ))}
                </div>
