...
export const PaperPreviewModal: React.FC<PaperPreviewModalProps> = ({
  ...
  // Dummy handlers for preview (no editing in preview mode)
  const dummyHandlers = {
    handleQuestionEdit: () => {},
    handleMCQOptionEdit: () => {},
    handleMarksChange: () => {},
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="min-w-[90%] max-w-[90%] max-h-[90vh] overflow-hidden p-0">
            ...
            <div className="flex items-center gap-4 pr-8">
              <div className="flex items-center space-x-2">
                <Switch
                  ...
                  checked={showAnswers}
                  onCheckedChange={setShowAnswers}
                  className='border border-gray-300 data-[state=unchecked]:bg-gray-400'
                />
                <Label htmlFor="show-answers" className="text-sm font-medium">
                  Show Answers
                </Label>
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-80px)] bg-gray-100 p-6">
          <div 
            ref={printRef}
            className="bg-white shadow-lg mx-auto pointer-events-none"
